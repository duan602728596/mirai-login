...
async function clean() {
  // 删除mac
  const macFiles = await globPromise(path.join(build, 'mac/mac/mirai-login.app/Contents/Resources/*.lproj'));
  ...
  // 重命名
  await fs.rename(path.join(build, 'mac/mac'), path.join(build, `mac/mirai-login-v${ version }-mac`));
  await fs.rename(path.join(build, 'win/win-unpacked'), path.join(build, `win/mirai-login-v${ version }-win`));
}
